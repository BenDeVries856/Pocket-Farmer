...
describe("JavaJdkInstaller telemetry", function () {
    ...
    before(function () {
        ...
        var tacoUtilsWithFakes = require("taco-utils"); // Reload taco utils with mocks
        ...
        tacoUtilsWithFakes = _.extend({}, tacoUtilsWithFakes, { TelemetryHelper: fakeTelemetryHelper, HasFakes: true }, fakeProcessUtilsModule);
    ...
    function telemetryGeneratedShouldBe(expectedTelemetry, expectedMessagePattern, done) {
        var javaJdkInstaller = new javaJdkInstallerModule(installerInfo, softwareVersion, installTo, new FakeLogger(), steps);
    ...
    describe("installation in mac", function () {
        var ChildProcessExecBehavior;
        (function (ChildProcessExecBehavior) {
            ChildProcessExecBehavior[ChildProcessExecBehavior["PackageInstallerCommandFailsScenario"] = 0] = "PackageInstallerCommandFailsScenario";
            ChildProcessExecBehavior[ChildProcessExecBehavior["PackageInstallerCommandCantBeRunScenario"] = 1] = "PackageInstallerCommandCantBeRunScenario";
            ChildProcessExecBehavior[ChildProcessExecBehavior["DetachingDmgFileFailsScenario"] = 2] = "DetachingDmgFileFailsScenario";
        })(ChildProcessExecBehavior || (ChildProcessExecBehavior = {}));
        var callTime;
        var childProcessExecBehavior;
        beforeEach(function () {
        ...
        function fakeExecToTestPackageInstallerFailure(command, optionsOrCallback, callback) {
            ...
            if (callTime === 0) {
                ...
                switch (childProcessExecBehavior) {
                    case ChildProcessExecBehavior.PackageInstallerCommandFailsScenario:
                        couldntRunPackageInstallerError.code = 54;
                        break;
        ...
        it("generates telemetry error when attaching the dmg file gives an error", function (done) {
            // child_process.exec call will fail for attach dmg, but succeed for detach dmg
            var timesCalled = 0;
            childProcessModule.exec = function (command, optionsOrCallback, callback) {
                if (callback === void 0) { callback = null; }
                var realCallback = (callback || optionsOrCallback);
                if (timesCalled++ === 0) {
                    // This is for the attach call
        ...
        it("generates telemetry error when the package installer gives an error", function (done) {
        ...
        it("generates telemetry error when the package installer can't be run", function (done) {
            // child_process.exec call will only fail for the install packages call
            childProcessModule.exec = fakeExecToTestPackageInstallerFailure;
        ...
        it("generates telemetry error when the detach dmg command fails", function (done) {
            // child_process.exec call will only fail for the install packages call
            childProcessModule.exec = fakeExecToTestPackageInstallerFailure;
    ...
    describe("installation in windows", function () {
        beforeEach(function () {
        ...
        it("generates telemetry if the installer ends with an error", function (done) {
            ...
            couldntRunInstallerError.code = 23;
        ...
        it("generates telemetry if the installer can't be run", function (done) {
