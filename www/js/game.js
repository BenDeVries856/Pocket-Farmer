...
function addClicks(){
  ...
  $('#cheat-button').off('click').on('click', function(){
    var popup = new Popup('reset', 'Are you sure?', true, function(){}, function(){
      resetGame();
    });
  });
...
// function resets the state of the game to the beginning
function resetGame(){
  money = 0;
  firstPlay = true;
  invAnimals = [];
  ...
  for(var i = 0; i < 4; i++){
    $('#ritems' + (i + 1)).click(function(){
      ...
      if(activeRoom.type == 'pasture'){
        ...
        $('#animals-lbl').html(formatText(invSeeds[lblIndex].type));
      }
      setTimeout(function(){
        ...
        if(animalsAdded > minAnimal){
            ...
            if(GLOBAL_ANIMALS[i].type == 'animal'){
              $('<div class="buy-locked"><span class="buy-locked-img">' +
                '</span><span class="buy-locked-text"><br>Locked<br>' + formatText(GLOBAL_ANIMALS[i].name) +
                '</span></div>').insertBefore('#buy-prev');
